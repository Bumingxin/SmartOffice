...
- **🎨 霓虹深空主题**：支持浅色/深色/跟随系统三种模式，深色模式采用赛博朋克霓虹深空风格，含星空粒子背景、发光边框、荧光按钮等视觉效果。

---

## 🎨 主题切换

SmartOffice 支持三种主题模式：

| 模式 | 说明 |
|------|------|
| 浅色（默认） | 经典白色界面，适合日常使用 |
| 深色 | 霓虹深空赛博朋克风格，星空粒子背景 + 发光边框 |
| 跟随系统 | 自动匹配操作系统的深色/浅色设置 |

**切换方式**：进入 设置 → 通用设置 → 外观，选择你喜欢的主题。

**持久化**：主题选择会自动保存到本地存储，刷新页面或重新打开浏览器后仍然生效。
...
深色模式采用霓虹深空赛博朋克风格，包含以下视觉效果：

- **星空粒子背景**：静态星点 + 漂浮碎片 + 弱光轨迹，纯 CSS 动画实现
- **霓虹发光边框**：青蓝色和紫红色双色霓虹效果，用于侧边栏、输入框、日期分隔线
- **荧光发送按钮**：紫色渐变发光按钮
- **金属标题牌**：SmartOffice / OpenClaw 标题带霓虹发光效果
- **AI 回复区分**：思考过程（粉色边框）和正式回复（青蓝色边框）用不同颜色区分

**技术实现**：
- 基于 CSS 自定义属性 + `data-theme` 属性切换
- 纯 CSS 动画，无 JavaScript 依赖，不影响性能
- 移动端保持原有设计，不受影响
